...
class OrcaMesh(metaclass=abc.ABCMeta):
    ...
    def _get_corner_dict(self):
        """ Get an info dictionary about the relative arranging (center, vertex, symmetry for the edges) of the des
ired gridpoint."""
        nodetype = self.stagg
...
def get_args():
    ...
    parser.add_argument('--xesmf', action=argparse.BooleanOptionalAction,
                        help="generate xesmf-type of file, with bounds stored as (y+1, x+1) array, instead of CF-co
mpliant (y,x,4) bounds")
